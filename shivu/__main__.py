...
from shivu.modules.setrarity import get_disabled_rarities, get_locked_character_ids

# Import all modules declared in ALL_MODULES
...
async def _update_user_info(user_id: int, tg_user: Any) -> None:
    try:
...
async def _update_group_user_totals(user_id: int, chat_id: int, tg_user: Any) -> None:
    try:
...
async def send_image(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    # Get chat's disabled rarities
    try:
        disabled_rarities = await get_disabled_rarities(chat_id)
    except Exception as e:
    ...
    try:
        locked_character_ids = await get_locked_character_ids()
    except Exception as e:
        LOGGER.exception(f"Failed to get locked characters: {e}")
        locked_character_ids = []

    try:
        # Fetch only characters with ENABLED rarities and NOT locked
        query = {}
...
async def guess(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if sorted(name_parts) == sorted(guess_text.split()) or any(part == guess_text for part in name_parts):
        ...
        # Update balance in user_collection directly
        try:
...
def main() -> None:
    # Setup setrarity command handlers

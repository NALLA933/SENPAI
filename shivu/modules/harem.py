"""
Harem/Collection Display Module
Handles showing user's character collection with pagination and filtering.
"""
...
from shivu.utils import RARITY_EMOJIS, to_small_caps, parse_rarity
...
async def harem_v3(update: Update, context: CallbackContext, page: int = 0):
    ...
    user, user_chars = await HaremManager.get_user_characters_fast(user_id, rarity_filter)
    ...
    char_details = await HaremManager.get_character_details_batch(page_ids)
    ...
    for cid in page_ids:
        if cid in char_details:
            ...
            name_rarity = parse_rarity(char_data.get('name', ''))
    ...
    anime_counts_task = asyncio.create_task(
        HaremManager.get_anime_counts_batch(page_animes)
    )

...
from shivu import application, user_collection, pm_users, LOGGER, SUPPORT_CHAT, UPDATE_CHAT, BOT_USERNAME
...
async def help_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    message = (
        f"<b>{to_small_caps('📖 Help')}</b>\n\n"
        f"<b>{to_small_caps('🎮 Game Commands:')}</b>\n"
        f"• /guess &lt;name&gt; - {to_small_caps('Guess the character')}\n"
        f"• /harem - {to_small_caps('View your collection')}\n"
        f"• /fav &lt;id&gt; - {to_small_caps('Set favorite character')}\n\n"
        f"<b>{to_small_caps('💰 Economy Commands:')}</b>\n"
        f"• /balance - {to_small_caps('Check your balance')}\n"
        f"• /pay &lt;user&gt; &lt;amount&gt; - {to_small_caps('Pay coins to user')}\n"
        f"• /shop - {to_small_caps('Buy characters')}\n"
        f"• /buy &lt;item&gt; - {to_small_caps('Buy from shop')}\n\n"
        f"<b>{to_small_caps('🔍 Search Commands:')}</b>\n"
        f"• /search &lt;name&gt; - {to_small_caps('Search characters')}\n"
        f"• /anime &lt;name&gt; - {to_small_caps('Search by anime')}\n"
        f"• /id &lt;id&gt; - {to_small_caps('Get character info')}\n\n"
        f"<b>{to_small_caps('📊 Leaderboard Commands:')}</b>\n"
        f"• /leaderboard - {to_small_caps('View leaderboards')}\n"
        f"• /top - {to_small_caps('Top 10 users')}\n"
        f"• /grouptop - {to_small_caps('Top groups')}\n\n"
        f"<b>{to_small_caps('🎁 Gift Commands:')}</b>\n"
        f"• /gift &lt;id&gt; &lt;user&gt; - {to_small_caps('Gift character')}\n"
        f"• /redeem &lt;code&gt; - {to_small_caps('Redeem a code')}\n\n"
        f"<b>{to_small_caps('⚙️ Settings:')}</b>\n"
        f"• /smode - {to_small_caps('Filter harem by rarity')}\n"
    )

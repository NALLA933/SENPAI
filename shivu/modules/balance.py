"""
Balance and Payment System Module
Handles user balances, payments, and admin balance management.
"""
...
async def change_balance(user_id: int, amount: int) -> int:
    """Atomically change balance by `amount` in user_collection. Returns the new balance."""
    if amount == 0:
...
async def balance_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    message = f"💰 <b>{name}</b>'s {to_small_caps('Balance')}: <b>{bal:,}</b> ᴄᴏɪɴs"
...
async def pay_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if not context.args and not update.message.reply_to_message:
        await update.message.reply_text(f"Usage: /pay <amount>")
        return
    ...
    if now < next_allowed:
        ...
        await update.message.reply_text(to_small_caps(f"⏱️ ʏᴏᴜ ᴍᴜsᴛ ᴡᴀɪᴛ {remaining}s ʙᴇғᴏʀᴇ sᴛᴀʀᴛɪɴɢ ᴀɴᴏᴛʜᴇʀ ᴘᴀʏᴍᴇɴᴛ."))
        return
    ...
    if update.message.reply_to_message and len(context.args) == 1:
        ...
        if len(context.args) < 2:
            await update.message.reply_text(to_small_caps("Usage: /pay <reply> <amount>"))
            return
    ...
    if not target_id:
        await update.message.reply_text(to_small_caps("✘ ᴄᴏᴜʟᴅ ɴᴏᴛ ʀᴇsᴏʟᴠᴇ ᴛᴀʀɢᴇᴛ ᴜsᴇʀ. ᴜsᴇ ᴜsᴇʀ ɪᴅ, @ᴜsᴇʀɴᴀᴍᴇ ᴏʀ ʀᴇᴘʟʏ ᴛᴏ ᴛʜᴇɪʀ ᴍᴇssᴀɢᴇ."))
        return

    if target_id == sender.id:
        await update.message.reply_text(to_small_caps("✓ ʏᴏᴜ ᴄᴀɴɴᴏᴛ ᴘᴀʏ ʏᴏᴜʀsᴇʟғ."))
        return
    ...
    try:
        ...
        await update.message.reply_text(to_small_caps("✘ ɪɴᴠᴀʟɪᴅ ᴀᴍᴏᴜɴᴛ. ᴜsᴇ ᴀ ᴘᴏsɪᴛɪᴠᴇ ɪɴᴛᴇɢᴇʀ."))
        return

    if amount <= 0:
        await update.message.reply_text(to_small_caps("✘ ᴀᴍᴏᴜɴᴛ ᴍᴜsᴛ ʙᴇ ɢʀᴇᴀᴛᴇʀ ᴛʜᴀɴ ᴢᴇʀᴏ."))
        return

    # Check sender balance
    bal = await get_balance(sender.id)
    if bal < amount:
        await update.message.reply_text(to_small_caps(f"✘ ʏᴏᴜ ᴅᴏɴ'ᴛ ʜᴀᴠᴇ ᴇɴᴏᴜɢʜ ᴄᴏɪɴs. ʏᴏᴜʀ ʙᴀʟᴀɴᴄᴇ: {bal:,}"))
        return
...
async def pay_callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if not pending:
        try:
            await query.edit_message_text(to_small_caps("✖️ ᴛʜɪs ᴘᴀʏᴍᴇɴᴛ ʀᴇǫᴜᴇsᴛ ʜᴀs ᴇxᴘɪʀᴇᴅ ᴏʀ ɪs ɪɴᴠᴀʟɪᴅ."))
        except Exception:
            pass
        return
    ...
    if time.time() - created_at > PENDING_EXPIRY_SECONDS:
        try:
            await query.edit_message_text(to_small_caps("⏱️ ᴛʜɪs ᴘᴀʏᴍᴇɴᴛ ʀᴇǫᴜᴇsᴛ ʜᴀs ᴇxᴘɪʀᴇᴅ."))
        except Exception:
            pass
    ...
    if action == "pay_cancel":
        try:
            await query.edit_message_text(to_small_caps("✘ ᴘᴀʏᴍᴇɴᴛ ᴄᴀɴᴄᴇʟʟᴇᴅ ʙʏ sᴇɴᴅᴇʀ."))
        except Exception:
            pass
        pending_payments.pop(token, None)
        return
    ...
    if now < next_allowed:
        remaining = int(next_allowed - now)
        await query.edit_message_text(to_small_caps(f"⏱️ ʏᴏᴜ ᴍᴜsᴛ ᴡᴀɪᴛ {remaining}s ʙᴇғᴏʀᴇ ᴍᴀᴋɪɴɢ ᴀɴᴏᴛʜᴇʀ ᴘᴀʏᴍᴇɴᴛ."))
        pending_payments.pop(token, None)
        return
    ...
    if not success:
        try:
            await query.edit_message_text(to_small_caps("✘ ᴛʀᴀɴsᴀᴄᴛɪᴏɴ ғᴀɪʟᴇᴅ: ɪɴsᴜғғɪᴄɪᴇɴᴛ ғᴜɴᴅs ᴏʀ ɪɴᴛᴇʀɴᴀʟ ᴇʀʀᴏʀ."))
        except Exception:
            pass
        pending_payments.pop(token, None)
        return
...
async def admin_addbal_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if user_id != OWNER_ID and user_id not in SUDO_USERS:
        await update.message.reply_text(to_small_caps("✘ ɴᴏᴛ ᴀᴜᴛʜᴏʀɪᴢᴇᴅ."))
        return

    if len(context.args) < 2:
        await update.message.reply_text(to_small_caps("Usage: /addbal <user_id> <amount>"))
        return
    ...
    try:
        ...
        await update.message.reply_text(to_small_caps("✘ ɪɴᴠᴀʟɪᴅ ᴀʀɢᴜᴍᴇɴᴛs."))
        return
    ...
    try:
        ...
        await update.message.reply_text(to_small_caps("✘ ғᴀɪʟᴇᴅ ᴛᴏ ᴜᴘᴅᴀᴛᴇ ʙᴀʟᴀɴᴄᴇ."))
